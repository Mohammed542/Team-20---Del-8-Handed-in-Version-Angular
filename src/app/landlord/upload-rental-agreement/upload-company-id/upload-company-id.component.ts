...
export class UploadCompanyIDComponent implements OnInit {
  ...
  private readAndUploadFile(theFile: any) {
    ...
    reader.onload = () => {
      ...
      // POST to server
      this.service.UploadCompanyIDDoc(file).subscribe(resp => {
  ...
  openConfirmDialog() {
    const dialogRef = this.dialog.open(ConfirmationDialogComponent, {
      data: {
        title: 'Confirm',
        message:
          'Are you sure you want to upload this document as the ID for the Company?',
      },
    })
    .afterClosed()
    .subscribe((response) => {
      if (response) {
        ...
        const dialogRef = this.dialog.open(ConfirmationDialogComponent, {
          data: {
            title: 'Successful Company ID uploaded!',
            message:
              'Your Company ID file has successfully been uploaded.',
          },
        })
      }
      else
