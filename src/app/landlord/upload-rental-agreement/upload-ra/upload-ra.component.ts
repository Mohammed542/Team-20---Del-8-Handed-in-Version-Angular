...
@Component({
  templateUrl: './upload-ra.component.html',
  styleUrls: ['./upload-ra.component.scss']
})
export class UploadRAComponent implements OnInit {
  ...
  openConfirmDialog() {
    ...
    .subscribe((response) => {
      if (response) {
        ...
        const dialogRef = this.dialog.open(ConfirmationDialogComponent, {
          data: {
            title: 'Successful Rental Agreement uploaded!',
            message:
              'Your Rental Agreement file has successfully been uploaded.',
          },
        })
      }
      else
