import { UpCompanyID } from './../../../UpCompanyID';
...
import { UpTenantID } from 'src/app/UpTenantID';

@Component({
  templateUrl: './upload-tenant-id.component.html',
  styleUrls: ['./upload-tenant-id.component.scss']
})
export class UploadTenantIDComponent implements OnInit {
  ...
  private readAndUploadFile(theFile: any) {
    ...
    reader.onload = () => {
      ...
      // POST to server
      this.service.UploadTenantIDDoc(file).subscribe(resp => {
  ...
  openConfirmDialog() {
    ...
    .subscribe((response) => {
      if (response) {
        ...
        const dialogRef = this.dialog.open(ConfirmationDialogComponent, {
          data: {
            title: 'Successful Tenant ID uploaded!',
            message:
              'Your Tenant ID file has successfully been uploaded.',
          },
        })
      }
      else
