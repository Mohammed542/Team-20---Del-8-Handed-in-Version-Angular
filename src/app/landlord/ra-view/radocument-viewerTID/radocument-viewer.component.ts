...
export class RADocumentViewerComponentTID implements OnInit {
  RAID = sessionStorage['RentalAgreementID'];
  fileBase64;
  constructor(private router: Router, private service: MainService, private matSnackBar: MatSnackBar, private dialogRef: MatDialogRef<RADocumentViewerComponentTID>) { }

  ngOnInit(): void {
    this.service.GetFileTenantID(this.RAID).subscribe((res: any) => {
      if (res.Success) {
