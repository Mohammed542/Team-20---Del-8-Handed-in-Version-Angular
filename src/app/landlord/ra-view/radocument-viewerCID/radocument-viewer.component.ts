...
export class RADocumentViewerComponentCID implements OnInit {
  RAID = sessionStorage['RentalAgreementID'];
  fileBase64;
  constructor(private router: Router, private service: MainService, private matSnackBar: MatSnackBar, private dialogRef: MatDialogRef<RADocumentViewerComponentCID>) { }

  ngOnInit(): void {
    this.service.GetFileCompanyID(this.RAID).subscribe((res: any) => {
      if (res.Success) {
